"""
Ge and Cui formulation for 3 Robots
"""
import numpy as np
from matplotlib import pyplot as plt
from matplotlib.lines import Line2D

from geometry import Sphere, MovingRepulsiveSphere, RepulsiveSphere, Attractive

x_start_coord = np.array([[-9, -4, 9], 
                         [7, -4, -7]])

x_goal_coord = np.array([[0, 7, -3],
                         [-6, 7, 6]])
...
moving_robot1 = MovingRepulsiveSphere(robot1)
moving_robot2 = MovingRepulsiveSphere(robot2)
moving_robot3 = MovingRepulsiveSphere(robot3)
...
def run_alternating_steps_3r(plan1, plan2, plan3, x_start1, x_start2, x_start3, nb_steps, axes = None): 
    ...
    u_path3[:,0] = plan3.function(x_start2)
    ...
    for i in range(1, nb_steps):
        ...
        temp_x_path1 = prev_x_path1 - plan1.epsilon * grad1
        temp_x_path2 = prev_x_path2 - plan2.epsilon * grad2
        temp_x_path3 = prev_x_path3 - plan3.epsilon * grad3

        #Find velocities of robots, divided by 1 time step:
        vel_robot1 = temp_x_path1 - prev_x_path1
        vel_robot2 = temp_x_path2 - prev_x_path2
        vel_robot3 = temp_x_path3 - prev_x_path3

        #n_RO points from robot to obstacle, in this case from one robot to another. Obstacle - Robot
        n_RO_O2R1 = (temp_x_path2 - temp_x_path1) / np.linalg.norm(temp_x_path1 - temp_x_path2)
        n_RO_O3R1 = (temp_x_path3 - temp_x_path1) / np.linalg.norm(temp_x_path1 - temp_x_path3)
        
        n_RO_O1R2 = (temp_x_path1 - temp_x_path2) / np.linalg.norm(temp_x_path2 - temp_x_path1)
        n_RO_O3R2 = (temp_x_path3 - temp_x_path2) / np.linalg.norm(temp_x_path2 - temp_x_path3)

        n_RO_O1R3 = (temp_x_path1 - temp_x_path3) / np.linalg.norm(temp_x_path3 - temp_x_path1)
        n_RO_O2R3 = (temp_x_path2 - temp_x_path3) / np.linalg.norm(temp_x_path3 - temp_x_path2)

        #v_RO is found with (v(t) - v_obs(t) * n_RO)
        v_RO_O2R1 = (vel_robot1 - vel_robot2).T @ n_RO_O2R1
        v_RO_O3R1 = (vel_robot1 - vel_robot3).T @ n_RO_O3R1

        v_RO_O1R2 = (vel_robot2 - vel_robot1).T @ n_RO_O1R2
        v_RO_O3R2 = (vel_robot2 - vel_robot3).T @ n_RO_O3R2

        v_RO_O1R3 = (vel_robot3 - vel_robot1).T @ n_RO_O1R3
        v_RO_O2R3 = (vel_robot3 - vel_robot1).T @ n_RO_O2R3

        #Reassign MovingRepulsiveSphere attributes, O# when robot is obstacle, R# When robot at hand
        moving_robot1.set_attr(n_RO_O2R1, v_RO_O2R1, n_RO_O3R1, v_RO_O3R1)
        moving_robot2.set_attr(n_RO_O1R2, v_RO_O1R2, n_RO_O3R2, v_RO_O3R2)
        moving_robot3.set_attr(n_RO_O1R3, v_RO_O1R3, n_RO_O2R3, v_RO_O2R3)
        ...
        moving_robot1.update_center(current_x_path1)
        moving_robot2.update_center(current_x_path2)
        moving_robot3.update_center(current_x_path3)
...
def test_planner_3_robots(steps, rep_weight1, rep_weight2, rep_weight3):
    ...
    potential1 = {
        "x_goal": x_goal1,
        "repulsive_weight": rep_weight1, #200
        "shape": "quadratic",
    }
    potential2 = {
        "x_goal": x_goal2,
        "repulsive_weight": rep_weight2, #50
        "shape": "quadratic",
    }
    potential3 = {
        "x_goal": x_goal3,
        "repulsive_weight": rep_weight3, #400
        "shape": "quadratic",
    }
    ...
    total1 = Total(world, potential1, moving_robot2, moving_robot3)
    total2 = Total(world, potential2, moving_robot1, moving_robot3)
    total3 = Total(world, potential3, moving_robot1, moving_robot2)
    ...
    # Run the planner
    x_path1, x_path2, x_path3 = run_alternating_steps_3r(plan1, plan2, plan3, x_start1, 
                                                      x_start2, x_start3,nb_steps, ax)
    ...
    ax.set_title(f'Ge and Cui PFM Formulation \n Number of Steps: {nb_steps}')


    plt.show()

if __name__ == '__main__':
    test_planner_3_robots(1500, rep_weight1=200, rep_weight2=50, rep_weight3=400)
