"""
Ge and Cui formulation for 2 Robots.
"""
import numpy as np
from matplotlib import pyplot as plt
from matplotlib.lines import Line2D

from geometry import Sphere, RepulsiveSphere, Attractive, MovingRepulsiveSphere2R
...
moving_robot1 = MovingRepulsiveSphere2R(robot1)
moving_robot2 = MovingRepulsiveSphere2R(robot2)
...
class Total: 
    ...
    potential.repulsiveWeight
        ...
        if self.other_robot is not None:
            robot_obstacle = self.other_robot
...
def run_alternating_steps(plan1, plan2, x_start1, x_start2, nb_steps, axes = None): #Adjust logic to use run method from Planner, or adjust run method
    '''
    ...
    Returns:
    - A tuple containing two numpy arrays (x_path1, x_path2), 
    representing the paths for both robots.
    ...
    for i in range(1, nb_steps):
        ...
        #Find velocities of robots, invisibly divided by 1 per time step:
        ...
        n_RO1 = (temp_x_path2 - temp_x_path1) / np.linalg.norm(temp_x_path1 - temp_x_path2)
        n_RO2 = (temp_x_path1 - temp_x_path2) / np.linalg.norm(temp_x_path2 - temp_x_path1)

        #v_RO is found with (v(t) - v_obs(t) * n_RO)
        v_RO1 = (vel_robot1 - vel_robot2).T @ n_RO1
        v_RO2 = (vel_robot2 - vel_robot1).T @ n_RO2

        #Reassign MovingRepulsiveSphere attributes
        moving_robot1.set_attr(n_RO1, v_RO1)
        moving_robot2.set_attr(n_RO2, v_RO2)
...
def test_planner_2_robots(steps, rep_weight1, rep_weight2):
    ...
    world = SphereWorld()  # Create a SphereWorld instance
    epsilon = 1e-3 #1e-3
    nb_steps = steps #2000
    ...
    potential1 = {
        "x_goal": x_goal1,
        "repulsive_weight": rep_weight1, #250 does the thing
        "shape": "quadratic",
    }
    potential2 = {
        "x_goal": x_goal2,
    ...
    total1 = Total(world, potential1, moving_robot2)
    total2 = Total(world, potential2, moving_robot1)
    plan1 = Planner(total1.eval, total1.grad, epsilon, nb_steps)
    plan2 = Planner(total2.eval, total2.grad, epsilon, nb_steps)

    # Run the planner
    x_path1, x_path2 = run_alternating_steps(plan1, plan2, x_start1, x_start2, nb_steps, ax)
...
if __name__ == '__main__':
    test_planner_2_robots(1000, rep_weight1=250, rep_weight2=1)
